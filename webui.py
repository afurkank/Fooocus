...
############################################# JOTFORM #####################################################
###########################################################################################################
from jotform_utils.log_image import log_image
###########################################################################################################
############################################# JOTFORM #####################################################
###########################################################################################################
...
############################################# JOTFORM #####################################################
###########################################################################################################
def _log_image(image_bytes, rating, user: str, img_model: str, 
               prompt: str, negative_prompt: str, aspect_ratio: str,# TODO: might add image type here
               sampling_method: str, schedule_type: str, cfg_scale: float,
               seed: int, sampling_steps: int, form_id: int):
    """
    ...
    TODO: this will take a gallery(gr.Gallery) of images instead
    """
    ...
    image_name = user + str(time.time()) # json.loads(generation_info.replace("\n", "\\n")).get('job_timestamp') # TODO: info is no longer str, it's dict
    # we don't know if images generated by fooocus returns timestamp
    # we can get the time directly at the time of logging, maybe name = " 'user name' + time "
    if rating is None:
        return gr.update(value="Please provide a rating.", visible=True)

    try:
        log_result = log_image(image_bytes, image_name, rating, generation_info, user, form_id=form_id)
        return gr.update(value=log_result, visible=True)
    except Exception as e:
        return gr.update(value=f"Error logging image: {str(e)}", visible=True)

# def generate_info(image_type:str, img_model:str, prompt:str, negative_prompt:str, 
#                   aspect_ratio: str, sampling_method:str, schedule_type:str,
#                   cfg_scale:float, seed:float, sampling_steps:int) -> dict:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1, visible=modules.config.default_advanced_checkbox) as advanced_column:
            ...
            with gr.Tab(label='Jotform'):
                ...
                with gr.Row():
                    llm_model = gr.Dropdown(["gpt-3.5-turbo"], value="gpt-3.5-turbo", label="Prompt Model", scale=1)
                
                # After the generation:
                # User
                # Rating
                # Log button
                # Other things related to icon
        ...
        # TODO: .then(update log, user, and rating rows for logging and rating)
        ...
        generate_button.click(lambda: (gr.update(visible=True, interactive=True), gr.update(visible=True, interactive=True), gr.update(visible=False, interactive=False), [], True),
            ...
            .then(lambda: (gr.update(visible=True, interactive=True), gr.update(visible=True, interactive=True), gr.update(visible=True, interactive=True), gr.update(visible=True, interactive=False)),
                  outputs=[rating_row, user_row, log_row, success_row]) \
            .then(fn=lambda: None, _js='playNotification').then(fn=lambda: None, _js='refresh_grid_delayed')
        
        ###########################################################################################################
        ############################################# JOTFORM #####################################################
        ###########################################################################################################
        log_button.click(fn=_log_image, inputs=[gallery, rating, user, base_model, prompt, # TODO: might add image type here
                                                negative_prompt, aspect_ratios_selection, sampler_name,
                                                scheduler_name, guidance_scale, image_seed,
                                                overwrite_step, form_id])
        ###########################################################################################################
        ############################################# JOTFORM #####################################################
        ###########################################################################################################
